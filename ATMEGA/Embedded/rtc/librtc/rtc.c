...
void rtc_set_alarm_once_per_sec() {
    //Enable Alarm Interrupt
    uint8_t val = rtc_read_byte(0x0e);
    rtc_write_byte(val | 0b00000100, 0x0e);

    //Set mask to alarm once per second
    rtc_write_byte(0b00000010, 0x07); // second
    rtc_write_byte(0b00000000, 0x08); // minute
    rtc_write_byte(0b10000000, 0x09); // hour
    rtc_write_byte(0b10000000, 0x0a); // day (upper bit must be set)


    // clear alarm flag
    val = rtc_read_byte(0x0f);
