...
void adc0832_init() {
    _ADC0832_CS_DDR  |= _BV(_ADC0832_CS);
    _ADC0832_CLK_DDR |= _BV(_ADC0832_CLK);
    _ADC0832_DO_DDR  &= ~_BV(_ADC0832_DO);
    _ADC0832_DI_DDR  |= _BV(_ADC0832_DI);

    // Deselect chip
    _ADC0832_CS_PORT |= _BV(_ADC0832_CS);
}

uint8_t adc0832_convert(uint8_t* lsb_ret, int ch) {
    int i;
    uint8_t bit;
    uint8_t msb = 0;
    uint8_t lsb = 0;
