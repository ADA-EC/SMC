...
int main(void) {
	...
	while(1) {
        ...
        while(uart_enabled) {
            uint8_t comando[50];
            _delay_ms(100);
            uint8_t flag = uart_read_line_int(comando, 50, &PIND, PIND3, 0);
            if (flag) {
                //uart_print("flag\n");
                PORTB &= ~_BV(PORTB0);
            ...
            else if(strcmp(comando, "la")==0){ //leituraatual
                double temp;// = 25.2;
                uint8_t humidity;// = 50;
                uint8_t methane;// = 30;
...
//Le todo o arquivo relativo à leira referenciada e o transmite por uart
//OBS: não será responsabilidade dessa função habilitar ou desabilitar interrupções
...
int LerArquivoTodoEPassarPorBluetooth(int Leira){
    ...
    //O vetor bufferLeitura é alocado com uma posição a mais para armazenar o '\0' necessário para transmissão uart
    ...
    if (f_open(fp, nomeArquivo, FA_READ) == FR_OK) {	// tenta abrir o arquivo da leira desejada para leitura
        ...
        while(br != 0){
            ...
            //atribui '\0' na ultima posição de bufferLeitura para que uart_print reconheça o fim da string
            bufferLeitura[br] = '\0';

            //transmite por uart a string bufferLeitura
            uart_print(bufferLeitura);
